...
app = FastAPI(title="Filter Agent")
...
#   _identity = load_agent_identity("filter_agent")
...
async def startup() -> None:
    tracer = get_tracer("filter-agent")
    with tracer.start_as_current_span("agent.bootstrap") as span:
        registry_url = os.getenv("REGISTRY_URL", "http://localhost:8000")
        agent_id = os.getenv("AGENT_ID", "filter-agent")
        try:
            ...
            agent_registry.register(
                ...
                endpoint=os.getenv("SERVICE_URL", "http://localhost:8003"),
            )
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent": "filter_agent"}


@app.post("/run", response_model=FilterAgentOutput)
