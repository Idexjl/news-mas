...
app = FastAPI(title="Search Worker Agent")
...
#   _identity = load_agent_identity("search_worker")
...
async def startup() -> None:
    tracer = get_tracer("search-worker")
    with tracer.start_as_current_span("agent.bootstrap") as span:
        registry_url = os.getenv("REGISTRY_URL", "http://localhost:8000")
        agent_id = os.getenv("AGENT_ID", "search-worker")
        try:
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent": "search_worker"}


@app.post("/run", response_model=SearchWorkerOutput)
@limiter.limit("30/minute")
async def run(request: Request, payload: SearchWorkerInput):
