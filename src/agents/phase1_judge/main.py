...
setup_telemetry("phase1-judge")
configure_langsmith()
logger = get_logger(__name__)

limiter = Limiter(key_func=get_remote_address)
app = FastAPI(title="Phase 1 Judge Agent")
...
#   _identity = load_agent_identity("phase1_judge")
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent": "phase1_judge"}


@app.post("/run", response_model=Phase1JudgeOutput)
