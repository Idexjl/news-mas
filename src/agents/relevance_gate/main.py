...
setup_telemetry("relevance-gate")
configure_langsmith()
logger = get_logger(__name__)

limiter = Limiter(key_func=get_remote_address)
app = FastAPI(title="Relevance Gate Agent")
...
#   _identity = load_agent_identity("relevance_gate")
...
async def startup() -> None:
    tracer = get_tracer("relevance-gate")
    with tracer.start_as_current_span("agent.bootstrap") as span:
        registry_url = os.getenv("REGISTRY_URL", "http://localhost:8000")
        agent_id = os.getenv("AGENT_ID", "relevance-gate")
        try:
            ...
            agent_registry.register(
                ...
                endpoint=os.getenv("SERVICE_URL", "http://localhost:8008"),
            )
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent": "relevance_gate"}


@app.post("/run", response_model=RelevanceGateOutput)
@limiter.limit("30/minute")
async def run(request: Request, payload: RelevanceGateInput):
